...
pub use node_types::{NodeType, OffPV, OnPV, Root};
...
impl SearchStats {
    ...
    pub fn print_profiling(&self) {
        ...
        if total_ns > 0 {
            ...
            println!(
                "profiling: gen {}% eval {}% order {}% other {}%",
...
impl Searcher {
    ...
    pub fn should_stop(&self) -> bool {
        ...
        if self.stats.nodes & 511 == 0 {
            ...
            if self.time_manager.hard_limit_exceeded()
                || (self.shared.ponderhit.load(Ordering::Relaxed)
                    && self.time_manager.elapsed() >= self.time_manager.hard_limit_ms())
            {
                self.shared.stop.store(true, Ordering::Relaxed);
                return true;
            }
    ...
    fn search_internal(&mut self, _limits: SearchLimits, max_depth: Depth) -> SearchResult {
        ...
        for depth in 1..=max_depth.raw() {
            ...
            self.shared
                .total_nodes
                .fetch_add(self.stats.nodes, Ordering::Relaxed);
