...
use super::Board;
use crate::types::{CastleRights, Color, File, Piece, Rank, Square};
...
impl Board {
    ...
    pub fn from_fen(fen: &str) -> Result<Board, &'static str> {
        ...
        board.castling = CastleRights::from_fen(parts[2]).ok_or("Invalid castling rights")?;
        ...
        board.ep_square = if parts[3] == "-" {
            ...
            let sq = Square::from_algebraic(parts[3]).ok_or("Invalid en passant square")?;
    ...
    pub fn to_fen(&self) -> String {
        ...
        for rank in (0..8).rev() {
            ...
            for file in 0..8 {
                let sq =
                    Square::from_file_rank(unsafe { File::from_index_unchecked(file) }, unsafe {
                        Rank::from_index_unchecked(rank)
                    });
