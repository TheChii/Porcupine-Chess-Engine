...
impl Board {
    ...
    pub fn generate_pawn_moves<M: MoveSink>(
        &self,
        moves: &mut M,
    ...
    fn add_promotions<M: MoveSink>(&self, moves: &mut M, from: Square, to: Square, capture: bool) {
        moves.push(Move::new(
            from,
            to,
            MoveFlag::promotion(Piece::Queen, capture),
        ));
        moves.push(Move::new(
            from,
            to,
            MoveFlag::promotion(Piece::Rook, capture),
        ));
        moves.push(Move::new(
            from,
            to,
            MoveFlag::promotion(Piece::Bishop, capture),
        ));
        moves.push(Move::new(
            from,
            to,
            MoveFlag::promotion(Piece::Knight, capture),
        ));
    }
    ...
    fn generate_en_passant<M: MoveSink>(&self, moves: &mut M, ep_sq: Square, _pinned: Bitboard) {
        ...
        for from in attackers {
            ...
            let king_sq = self.king_square(us);
            let occ =
                (self.occupied() ^ Bitboard::from_square(from) ^ Bitboard::from_square(cap_sq))
            ...
            let bishops =
                (self.piece_bb(Piece::Bishop) | self.piece_bb(Piece::Queen)) & self.them();
